...
- **`issue_add_comment`**: Добавить комментарий к задаче
  - Параметры:
    ...
    - `text` (строка, обязательно): Текст комментария (поддерживает разметку Яндекс.Трекера)
    - `attachment_ids` (массив строк, опционально): ID временных файлов для прикрепления к комментарию. Сначала используйте `attachment_upload_temp` для загрузки файлов
    - `summonees` (массив строк, опционально): Логины пользователей для упоминания в комментарии
    - `is_add_to_followers` (логический, опционально, по умолчанию: true): Добавлять ли автора комментария в наблюдатели задачи
  - Возвращает созданный объект комментария с id, текстом, автором и временными метками
...
- **`attachment_upload_temp`**: Загрузить временный файл в Яндекс.Трекер
  - Параметры:
    - `filename` (строка, обязательно): Имя загружаемого файла (например, "screenshot.png", "report.pdf")
    - `content_base64` (строка, обязательно): Содержимое файла, закодированное в base64
    - `mimetype` (строка, опционально): MIME-тип (например, "image/png", "application/pdf"). Если не указан, будет определён по имени файла
  - Возвращает объект вложения с ID, который можно использовать в `issue_create`, `issue_update` или `issue_add_comment` через параметр `attachment_ids`
